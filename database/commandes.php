...
function ajouterUser( $nom, $Téléphone, $Adresse_Electronique, $password)
{
  if(require("connexion.php"))
  {
    $req = $access->prepare("INSERT INTO utilisateur (nom, telephone, email, motdepasse) VALUES (?, ?, ?, ?)");

    $req->execute(array($nom, $Téléphone, $Adresse_Electronique, $password));
...
function afficherUnProduit($id)
{
	if(require("connexion.php"))
	{
		$req=$access->prepare("SELECT * FROM produit WHERE idcategory=?");
...
function afficher()
{
	if(require("connexion.php"))
	{
		$req=$access->prepare("SELECT * FROM=? ORDER BY id DESC");
